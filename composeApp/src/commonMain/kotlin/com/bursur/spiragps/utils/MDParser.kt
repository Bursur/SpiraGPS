...
fun parseMarkdownToAnnotatedString(markdown: String): AnnotatedString {
    ...
    addMatches(underlineRegex, TokenType.UNDERLINE, 1)
    addMatches(strikeThroughRegex, TokenType.STRIKE_THROUGH, 1)
    ...
    for (token in tokens) {
        ...
        when (token.type) {
            ...
            TokenType.UNDERLINE -> {
                val underlineContent = token.groups[0]
                val styleStart = builder.length
                builder.append(underlineContent)
                builder.addStyle(
            ...
            TokenType.STRIKE_THROUGH -> {
                val strikeThroughContent = token.groups[0]
                val styleStart = builder.length
                builder.append(strikeThroughContent)
                builder.addStyle(
